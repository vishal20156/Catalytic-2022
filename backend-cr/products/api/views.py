...
def product_image_upload(request):
	...
	if form.is_valid():
		...
		product_object = Product.objects.filter(name=form.cleaned_data["name"]).update(image=request.FILES['image'])
		return Response({"message":"saved", "product":product_object}, status=200)
